...
const navItems = [
  ...
  { href: "/admin/grade", label: "编程题评分", icon: "🎯" },
  { href: "/admin/records", label: "记录查看", icon: "📋" },
  { href: "/admin/stats", label: "统计数据", icon: "📊" },
];
    ...
    <>
      ...
      >
        <div className="p-5 border-b border-th-border flex items-center justify-between">
          <Link href="/admin" className="font-bold text-th-text font-mono text-sm">
            <span className="text-[#58a6ff]">admin</span>
            <span className="text-[#3fb950]">-panel</span>
          </Link>
          {/* 主题切换按钮 */}
          {mounted && (
            <button
        ...
        <div className="p-4 border-t border-th-border space-y-2">
          {session?.user && (
            <div className="flex items-center gap-2 px-1 mb-1">
              <div className="h-6 w-6 rounded-full bg-gradient-to-br from-[#58a6ff] to-[#bc8cff] flex items-center justify-center text-xs font-bold text-white shrink-0">
                {(session.user.displayName || session.user.name || "A").charAt(0).toUpperCase()}
              </div>
