  ...
  async function grade(isCorrect: boolean) {
    ...
    try {
      const res = await fetch("/api/admin/grade", {
        ...
        body: JSON.stringify({ type: item.type, answerId: item.id, isCorrect, comment: comment || null }),
      });
      if (res.ok) {
        setGraded(isCorrect);
        onGraded(item.id);
      ...
      <div className="flex items-center justify-between border-b border-th-border bg-th-bg2 px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <span className="h-7 w-7 rounded-full bg-[#bc8cff]/20 flex items-center justify-center text-xs font-bold text-[#bc8cff]">
...
export default function GradeClient({ initialExamCount, initialPracticeCount }: { initialExamCount: number; initialPracticeCount: number }) {
  ...
  const fetchData = useCallback(async (t: BankType, p: number, s: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/grade/list?type=${t}&page=${p}&search=${encodeURIComponent(s)}`);
